...
const CARDS = [
  {
    ...
    icon: (
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
        ...
        />
        <path
          d="M18 2.5l.5 1.5 1.5.5-1.5.5-.5 1.5-.5-1.5-1.5-.5 1.5-.5z"
          fill="#3a7a28"
        />
      </svg>
    ),
...
function pLen(a: [number, number], b: [number, number]) {
  return Math.hypot(b[0] - a[0], b[1] - a[1]);
}
function perim(pts: [number, number][]) {
  let s = 0;
  for (let i = 0; i < pts.length; i++) s += pLen(pts[i], pts[(i + 1) % pts.length]);
  return s;
}
...
export default function RoleplaySection() {
    ...
    function frame(ts: number) {
      ...
      if (inWin && !s.wasInWindow && !s.inTransition) {
        ...
        tipEl.style.transition =
          "opacity 0.55s cubic-bezier(.22,1,.36,1),transform 0.55s cubic-bezier(.22,1,.36,1)";
        ...
        s.inTransition = true;
        tipEl.style.transition =
          "opacity 0.42s ease-in,transform 0.42s cubic-bezier(.4,0,1,1)";
  ...
  return (
    ...
    >
      ...
      <div
        ref={sceneRef}
        style={{ position: "relative", top: 50, width: "100%", maxWidth: 760 }}
      >
